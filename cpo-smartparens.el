...
;;
;; Note, sometimes I may use the word symex.
;; Symex is just a newer name that I like for “symbolic expression”, IE “s-expression” or “sexp”, rhymes with other terms that have “expression” in them like regex (regular expression), redex (reducible expression).
;; Thanks Sid Kasivajhula!
;;
;; A note about positioning.
;; Point can be “at” a symex, “in” a symex, or both.
;; If it is both “at” and “in”, we prefer “at”.
;; IE `(|(a b) c)` is “at” the inner symex but “in” the outer symex.
;; There are 3 key cases for point being “at” a symex.
;; Point can be before a symex `|(...)`, after `(...)|`, or in sandwich position `(...)|(...)`.
;; In sandwich position it is sort of at two symexes, but we handle it by preferring the symex after point.
;; Sandwich position is rare in real Lisp code, but comes up when using smartparens for non-lisps when you get situations like a[](), a mix of index access and function calls.
;; So for `(a (b)|(c))`, for doing things like expanding region, transposing the symex at point, etc, we select the symex `(c)`.
;; If something doesn't behave that way, then it is a bug.
;; If a test (probably generated by capturing current behavior and not being sufficiently examined) specifies behavior that doesn't follow this, the test is wrong.
;;
...
"
  (let ((point (or point (point))))
    (save-mark-and-excursion
      (goto-char point)
      (let* ((close-delim (and (not (equal direction 'forward))
                               (or (equal direction 'backward)
                                   (and (not (cpo-smartparens--at-open-delimiter-p))
                                        (cpo-smartparens--at-close-delimiter-p)))))
             (thing (sp-get-thing (cond ((equal direction 'forward) nil)
                                        ((equal direction 'backward) t)
                                        ((cpo-smartparens--at-open-delimiter-p) nil)
                                        (close-delim t)
                                        ((cpo-smartparens--at-end-of-symbol-sexp-probably) t)
                                        (t nil))))
             ;; When at a close delimiter, sp-get-thing backward may
             ;; return a child symbol inside the delimited sexp rather
             ;; than the delimited sexp itself (this happens without
             ;; smartparens-mode active).  Detect this and use
             ;; scan-sexps to find the correct bounds.
             (thing (if (and close-delim thing
                             (equal "" (plist-get thing ':op))
                             (< (plist-get thing ':end) point))
                        (let ((open-pos (condition-case nil
                                            (scan-sexps point -1)
                                          (error nil))))
                          (if open-pos
                              (list ':beg open-pos ':end point
                                    ':op (and (stringp close-delim)
                                              (car (rassoc close-delim sp-pair-list)))
                                    ':cl (if (stringp close-delim) close-delim "")
                                    ':prefix "" ':suffix "")
                            thing))
                      thing))
...
(defun cpo-smartparens--up-sexp ()
  ;; When at a close delimiter, the sexp at point IS the delimited sexp
...
;;;###autoload (autoload 'cpo-smartparens-raise "cpo-smartparens.el" "" t)
(defun cpo-smartparens-raise ()
  "Replace the parent sexp with the current sexp at point.
This is the well-known raise-sexp operation: the child is promoted to take
the parent's place, discarding the parent and any siblings.
For example, with point at `c` in `(a (b c) d)`, raise produces `c`.
If at a delimiter (IE just before open or just after close), operates on the delimited sexp.
(Prefers selecting sexp after point if at two with no space.)
"
  (interactive)
  (let* ((child-bounds (cpo-smartparens--bounds-of-sexp-at-point (point) 'always))
         ;; When point is inside the detected child (not at its beginning),
         ;; check alternatives.  First, try forward: if there is a sexp
         ;; starting exactly at point (no space gap), prefer it -- this is
         ;; the "sandwich" case where point sits between a close delimiter
         ;; and the next sexp with no space.  Otherwise, try backward to
         ;; find a strictly smaller match (handles point just before a
         ;; closing delimiter where the forward detection picks up the
         ;; enclosing sexp).
         (child-bounds (if (and child-bounds
                                (< (car child-bounds) (point)))
                           (let ((fwd (cpo-smartparens--bounds-of-sexp-at-point
                                       (point) 'always 'forward))
                                 (bwd (cpo-smartparens--bounds-of-sexp-at-point
                                       (point) 'always 'backward)))
                             (cond
                              ;; Prefer forward sexp starting exactly at point
                              ((and fwd (= (car fwd) (point)))
                               fwd)
                              ;; Fall back to backward if strictly smaller
                              ((and bwd
                                    (cpo-tree-walk--region-strictly-less bwd child-bounds))
                               bwd)
                              (t child-bounds)))
                         child-bounds))
         (parent-bounds (and child-bounds
                             (save-mark-and-excursion
                               ...
                               (when (cpo-tree-walk--motion-moved 'cpo-smartparens-up-parent-beginning)
                                 (let ((pb (cpo-smartparens--bounds-of-sexp-at-point (point) 'never)))
                                   (when (and pb (cpo-tree-walk--region-strictly-less child-bounds pb))
                                     pb)))))))
    (when (and child-bounds parent-bounds
               (cpo-tree-walk--region-strictly-less child-bounds parent-bounds))
      (let ((child-text (buffer-substring-no-properties (car child-bounds) (cdr child-bounds)))
            (at-end (= (point) (cdr child-bounds))))
        (atomic-change-group
          (delete-region (car parent-bounds) (cdr parent-bounds))
          (goto-char (car parent-bounds))
          (insert child-text))
        (if at-end
            (goto-char (+ (car parent-bounds) (length child-text)))
          (goto-char (car parent-bounds)))))))
